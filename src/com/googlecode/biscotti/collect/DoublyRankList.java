 ...
 * storing items (in this case indices) in sorted order. Strictly speaking it is
 * impossible to make any hard guarantees regarding the worst-case performance
 * of this class. Practical performance is <i>expected</i> to be logarithmic
 * with an extremely high degree of probability as the list grows.
 * <p>
 ...
 */
public class DoublyRankList<E> extends AbstractList<E> implements List<E>,
	...
	private DoublyRankList() {
		Arrays.fill(head.distance, 1);
		Arrays.fill(head.next, tail);
		Arrays.fill(tail.prev, head);
		//tail.prev = head;
	}

	public static <E extends Comparable<? super E>> DoublyRankList<E> create() {
		return new DoublyRankList<E>();
	}

	public static <E> DoublyRankList<E> create(final Iterable<? extends E> elements) {
		checkNotNull(elements);
		DoublyRankList<E> list = new DoublyRankList<E>();
...
//		newNode.next[0].prev = newNode;
	...
	public ListIterator<E> listIterator() {
		return new ListIterator<E>() {
			...
			public void add(E element) {
				...
				DoublyRankList.this.add(index++, element);
			...
			public void remove() {
				...
				DoublyRankList.this.remove(--index);
				expectedModCount = modCount;
				last = null;
			}
	 ...
	 */
	@Override
	public DoublyRankList<E> clone() {
		DoublyRankList<E> clone;
		try {
			clone = (DoublyRankList<E>) super.clone();
		} catch (java.lang.CloneNotSupportedException e) {
			throw new InternalError();
		}
		//clone.head = new Node<E>(null, MAX_LEVEL);
		Arrays.fill(clone.head.next, clone.tail);
		Arrays.fill(clone.tail.prev, clone.head);
