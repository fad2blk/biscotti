 ...
 * used to order the elements in the collection. Essentially this allows another
 * sorted collection of a desired type to create a copy of this collection.
 * (Note: when using <i>natural ordering</i> the comparator method may return a
 * natural order comparator or {@code null} depending on the specific
 * implementation.)
 * <p>
 * In addition all implementation classes are expected to provide three static
 * creation methods: {@code create()}, returning a collection that orders its
 * elements according to their <i>natural ordering</i>,
 * {@code create(Comparator)} returning a collection which uses the specified
