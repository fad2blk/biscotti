 ...
 */
public class CopyOfTreeList<E> extends AbstractList<E> implements
		SortedList<E>, Serializable {
	...
	protected CopyOfTreeList(){};

	private CopyOfTreeList(final Comparator<? super E> comparator) {
		if (comparator != null)
	...
	private CopyOfTreeList(final Iterable<? extends E> elements) {
	 ...
	 *         their natural ordering
	 */
	public static <E> CopyOfTreeList<E> create() {
		return new CopyOfTreeList<E>((Comparator<? super E>) null);
	}
	...
	public static <E> CopyOfTreeList<E> create(
		...
		return new CopyOfTreeList<E>(comparator);
	}
	...
	public static <E> CopyOfTreeList<E> create(
		...
		return new CopyOfTreeList(elements);
	}
	...
	public List<E> subList(int fromIndex, int toIndex){
		...
		for(;i < toIndex; i++ )
			maxNode = successor(maxNode);
		return new SubList(minNode, maxNode, root, size);
	...
	protected Node search(final E e) {
		Node node = root;
		while (node != null && e != null && node != predecessor(min)
				&& node != successor(min)) {
	...
	private class SubList extends CopyOfTreeList<E>{
		...
		SubList(Node min, Node max, Node root, int size){
			this.min = min;
			this.max = max;
			this.root = root;
			this.size = size;
			
			System.out.println("min:" + this.min.element + " max:" + this.max.element
					+ " size:" + this.size);
