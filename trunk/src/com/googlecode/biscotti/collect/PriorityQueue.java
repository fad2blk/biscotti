...
public class PriorityQueue<E> extends AbstractQueue<E> implements
	...
	protected Node max = nil;
	protected Node min = nil;
	...
	public boolean offer(E element) {
		...
		Node newNode = new Node(element, nil, nil, nil);
		if (root == nil)
			min = max = root = newNode;
		else {
			int cmp;
			Node parent;
			Node t = root;
	...
	class Node {
		...
		Node(final E element, final Node parent, final Node left,
				final Node right) {
			this.left = left;
			this.right = right;
			this.parent = parent;
	...
	private void fixAfterDeletion(Node node) {
		...
		while (node != root && node.color == Color.BLACK) {
			if (node == node.parent.left) {
				...
				if (n.right.color == Color.BLACK && n.left.color == Color.BLACK) {
					n.color = Color.RED;
					node = node.parent;
				} else {
					if (n.left.color == Color.BLACK) {
						n.left.color = Color.BLACK;
						n.right.color = Color.BLACK;
						n.color = Color.RED;
						rotateLeft(n);
